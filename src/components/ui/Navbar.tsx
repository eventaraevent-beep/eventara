...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-gradient-to-b from-rich-black/95 to-burgundy-dark/80 backdrop-blur-md border-b border-event-gold/20">
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`font-inter text-sm font-medium transition-colors ${
                  pathname === item.href
                    ? 'text-event-gold'
                    : 'text-cream-light/90 hover:text-event-gold'
                }`}
              >
                {item.label}
              </Link>
          ...
          >
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="block px-4 py-2 text-cream-light/90 hover:text-event-gold transition-colors"
